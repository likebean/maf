...
export default function App(): JSX.Element {
  const backendUrl = import.meta.env.VITE_BACKEND_URL ?? "http://127.0.0.1:8892";
  ...
  const runWithPayload = async (payload: Record<string, unknown>): Promise<void> => {
    const existingForwarded =
      payload.forwardedProps && typeof payload.forwardedProps === "object" && !Array.isArray(payload.forwardedProps)
        ? (payload.forwardedProps as Record<string, unknown>)
        : payload.forwarded_props &&
            typeof payload.forwarded_props === "object" &&
            !Array.isArray(payload.forwarded_props)
          ? (payload.forwarded_props as Record<string, unknown>)
          : {};

    const body: Record<string, unknown> = {
      ...payload,
      thread_id: payload.thread_id ?? threadIdRef.current,
    ...
    activeRunIdRef.current = typeof body.run_id === "string" ? body.run_id : null;
    pendingUsageRef.current = null;
    setIsRunning(true);
  ...
  const startNewTurn = async (text: string): Promise<void> => {
    if (caseClosedRef.current && pendingInterrupts.length === 0) {
      // Backend recreates the workflow instance for a new case on this thread.
          ...
          <article className="card interrupt-card">
            ...
            {!currentInterrupt && (
              <div className="pending-empty-state">
                <p className="muted">
                  {isCaseClosed
                    ? "This case is complete. Send another message to start a new workflow instance on this thread, or Start New Case for a fresh thread id."
              ...
              placeholder={
                currentInterruptKind === "approval"
                  ...
                  : currentInterruptKind === "handoff_input"
                    ? "Reply to continue..."
                    : isCaseClosed
                      ? "Case complete — send another message to recreate the workflow, or Start New Case..."
                      : "Describe your issue..."
              }
